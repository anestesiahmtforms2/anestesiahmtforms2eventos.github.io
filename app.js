...
const SIGLA_ALIASES = {
  ...
  AM: ["Alexandre de Castro Morais"],
  BA: ["Barbara Ribeiro Coutinho Leduc"],
  BG: ["Bernardo Guimaraes"],
  CA: ["Carlos Humberto Barbosa Ganem"],
  CR: ["Crelio Viana"],
  DE: ["Deiler Celio Jeunon"],
  DN: ["Dener Augusto Diniz"],
  FL: ["Flavio Maciel Fonseca"],
  FR: ["Francisco Tadeu da Mota Albuquerque"],
  GB: ["Gustavo Prosperi Bicalho", "Guilherme Vieira Cunha"],
  GU: ["Guilherme Vieira Cunha"],
  IG: ["Igor Fagundes Vieira"],
  JA: ["Jayme Bueno Castilho"],
  JK: ["Jessica Karine"],
  LC: ["Lucas Cardoso de Andrade", "Luciano Costa Ferreira"],
  LD: ["Leonardo Diniz Correa Pinto", "Luiz Otavio Fernandes Andrade"],
  LE: ["Leonardo Alves Araujo"],
  LH: ["Lucia Helena Jacomett"],
  LM: ["Lucas Marques"],
  LO: ["Luciano Costa Ferreira"],
  LU: ["Luiz Antonio Carneiro Silva"],
  MA: ["Marcelo Giovannoni Assis"],
  MH: ["Marcio Henrique Mendes"],
  PR: ["Paulo Renato Andrade Silva"],
  RA: ["Rafael Augusto Carneiro Rezende", "Rodrigo de Lima e Souza"],
  RC: ["Ricardo Lucas da Mota Albuquerque", "Rodrigo Capuano de Rezende Carneiro"],
  RO: ["Rodrigo Capuano de Rezende Carneiro", "Rodrigo de Lima e Souza"],
  RU: ["Rubens Claudio Pinheiro"],
  WE: ["Wendell Valadares Campos Pereira"],
  L2: ["Leonardo Carvalho Figueiredo"],
};
...
const backToAppEditButton = document.getElementById("backToAppEditButton");
...
let activeScheduleTokenKey = "";
let activeScheduleTokenDate = "";
let activeScheduleTokenButton = null;
...
function resolveAusenteNameForToken(token, selectedName = "") {
  const normalizedSelected = normalizeText(selectedName);
  if (normalizedSelected) {
    const selectedOption = currentAusenteOptions.find((value) => normalizeText(value) === normalizedSelected);
    if (selectedOption) {
  ...
  if (normalizedToken === "DC") {
    return "DC";
  }

  const directMatch = currentAusenteOptions.find((value) => normalizeToken(value) === normalizedToken);
  if (directMatch) {
    return directMatch;
  }

  const aliasMatches = SIGLA_ALIASES[normalizedToken];
  if (aliasMatches?.length) {
    const preferredAlias = aliasMatches
      .map((alias) => currentAusenteOptions.find((value) => normalizeText(value) === normalizeText(alias)))
      .find(Boolean);
    if (preferredAlias) {
  ...
  const initialsMatch = currentAusenteOptions.find((value) => getInitials(value) === normalizedToken);
  if (initialsMatch) {
    return initialsMatch;
  }

  const tokenStartMatch = currentAusenteOptions.find((value) => normalizeToken(value).startsWith(normalizedToken));
  if (tokenStartMatch) {
    return tokenStartMatch;
  }

  return selectedName || token;
}
...
function formatLatestSyncedText(record) {
  ...
  return `Data do Evento: ${record.data || "-"} | Tipo de Evento: ${record.evento || "-"} | Membro Ausente/Atrasado: ${
    record.ausente || "-"
  } | Substituto: ${record.presente || "-"} | Turno: ${record.turno || "-"} | Pagador: ${
    record.devedor || "-"
  } | Credor: ${record.credor || "-"} | Valor a Pagar: ${record.valorPagar || "-"}`;
}
...
function renderLatestSynced(record = readLatestSyncedCache()) {
  ...
  card.innerHTML = `
    <p class="history-kv-line">${formatLatestSyncedText(record)}</p>
    ${historyLine}
  `;
  latestSyncedCard.appendChild(card);
}
...
function renderRegisteredRecords(dateKey = registeredDateFilter?.value || getLocalDateString()) {
  ...
  records.forEach((record) => {
    ...
    card.innerHTML = `<p class="history-kv-line">${formatLatestSyncedText(record)}</p>${historyLine}`;
...
function renderMonthlySummary() {
  ...
  const records = monthlySummaryRecords
    ...
    .sort((left, right) => {
      const byDate = String(left.data || left.dataDoEvento || "").localeCompare(String(right.data || right.dataDoEvento || ""));
      if (byDate !== 0) {
        return byDate;
      }
...
function createStyledSummaryPdfBlob(monthKey, records) {
  ...
  const columns = [
    { label: "DATA(S) DO EVENTO", key: "datas", width: 150, maxLength: 30 },
    { label: "EVENTO", key: "evento", width: 98, maxLength: 18 },
    { label: "DEVEDOR", key: "devedor", width: 108, maxLength: 18 },
    { label: "CREDOR", key: "credor", width: 108, maxLength: 18 },
    { label: "TOTAL DEVIDO", key: "total", width: usableWidth - 150 - 98 - 108 - 108, maxLength: 13 },
  ];
  ...
  records.forEach((record, index) => {
    const rowCells = columns.map((column) => {
      const value =
        column.key === "datas"
          ? formatPdfDate(record.data || record.dataDoEvento || record.datas)
          : column.key === "evento"
            ? record.evento || "-"
          : record[column.key];
      return wrapPdfText(value, column.maxLength);
    });
...
function formatPdfDate(dateKey) {
  if (!dateKey) {
    return "-";
  }
...
function renderScheduleByDate(dateKey) {
  ...
  if (registeredDateFilter && registeredDateFilter.value !== dateKey) {
  ...
  day.siglas.forEach((token, index) => {
    ...
    tokenNode.addEventListener("click", () => openEventForSigla(token, day.date, isVacation, tokenNode));

    const tokenKey = normalizeToken(token);
    if (activeScheduleTokenDate === day.date && activeScheduleTokenKey === tokenKey) {
      tokenNode.classList.add("sigla-token--armed");
    }
...
function openEventForSigla(token, dateKey, isVacation, tokenButton) {
  ...
  const tokenChoices = rawToken.split(/[\\/|-]/).map((value) => value.trim()).filter(Boolean);
  const choices = normalizeToken(rawToken) === "DC" ? dcChoices : tokenChoices;
  let selected = choices[0] || rawToken;
  if (isVacation || tokenChoices.length > 1 || normalizeToken(rawToken) === "DC") {
    const answer = window.prompt(
      `Escolha o membro para registrar:\n${choices.map((value, index) => `${index + 1} - ${value}`).join("\n")}`,
      "1",
    );
  ...
  activeScheduleTokenKey = normalizeToken(rawToken);
  activeScheduleTokenDate = dateKey;
  activeScheduleTokenButton = tokenButton || null;
  activeScheduleTokenButton?.classList.remove("sigla-token--highlight");
  activeScheduleTokenButton?.classList.add("sigla-token--armed");
  const selectedName = String(selected || "").split(" - ").pop()?.trim() || String(selected || "").trim();
  const resolvedAusente = resolveAusenteNameForToken(rawToken, selectedName);
  const matchingAusente = currentAusenteOptions.find((value) => normalizeText(value) === normalizeText(resolvedAusente));
  if (matchingAusente) {
    ausenteSelect.value = matchingAusente;
  } else {
    ausenteSelect.value = "";
  }
  ausenteSelect?.dispatchEvent(new Event("change", { bubbles: true }));
  renderScheduleByDate(dateKey);
  eventEntryCard?.scrollIntoView({ behavior: "smooth", block: "start" });
  updateEventoState();
  updateFieldHaloState();
}

function clearActiveScheduleToken() {
  activeScheduleTokenKey = "";
  activeScheduleTokenDate = "";
  activeScheduleTokenButton?.classList.remove("sigla-token--armed");
  activeScheduleTokenButton = null;
  renderScheduleByDate(scheduleDateInput?.value || getLocalDateString());
}
...
function openEditModal() {
  ...
  if (editObservacoesInput) {
    editObservacoesInput.value = latestSyncedRecord.observacoes || latestSyncedRecord.observacao || "";
  }
...
function returnToAppFromLaunch() {
  clearActiveScheduleToken();
  resetForm();
  eventEntryCard?.classList.add("hidden");
  renderScheduleByDate(scheduleDateInput?.value || getLocalDateString());
  scheduleSiglasGrid?.scrollIntoView({ behavior: "smooth", block: "start" });
}

function returnToAppFromEdit() {
  clearActiveScheduleToken();
  closeEditModal();
  renderScheduleByDate(scheduleDateInput?.value || getLocalDateString());
  scheduleSiglasGrid?.scrollIntoView({ behavior: "smooth", block: "start" });
...
scheduleDateInput?.addEventListener("change", () => {
  const dateKey = clampScheduleDate(scheduleDateInput.value || getLocalDateString());
  scheduleDateInput.value = dateKey;
  if (registeredDateFilter) {
    registeredDateFilter.value = dateKey;
    renderRegisteredRecords(dateKey);
...
registeredDateFilter?.addEventListener("change", () => {
  const dateKey = registeredDateFilter.value || getLocalDateString();
  if (scheduleDateInput && scheduleDateInput.value !== dateKey) {
    scheduleDateInput.value = dateKey;
    renderScheduleByDate(dateKey);
  }
  renderRegisteredRecords(dateKey);
...
backToAppButton?.addEventListener("click", returnToAppFromLaunch);
backToAppEditButton?.addEventListener("click", returnToAppFromEdit);
authForm?.addEventListener("submit", handleAuthSubmit);
passwordInput?.addEventListener("input", sanitizePasswordInput);
window.addEventListener("focus", () => {
  if (isAuthenticated()) {
    fetchBootstrapData().catch(() => {});
    renderRegisteredRecords(registeredDateFilter?.value || getLocalDateString());
  }
});
